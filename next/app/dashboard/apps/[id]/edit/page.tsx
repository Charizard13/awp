...
export default async function EditAppPage({ params }: { params: { message: string; id: string } }) {
  const cookieStore = cookies();
  const supabase = createServerClient(cookieStore);

  const { error: appError, data: app } = await supabase.from("apps").select().eq("id", params.id).single();
  ...
  const handleSubmitEditApp = async (formData: FormData) => {
    ...
    const { error: updateError } = await supabase.from("apps").update({ name, description, url }).match({ id: app.id });

    if (updateError) {
      return redirect(`${route}?message=There was an error updating your metadata.`);
    }
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Image src={iconUrl} alt={app.name} className="w-16 h-16 rounded-full" width={100} height={100} />
                <p className="text-sm text-zinc-500 dark:text-zinc-400">Upload your app&apos;s icon.</p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="app-name">Name</Label>
                <Input id="app-name" placeholder="App Name" required name="name" maxLength={50} minLength={3} defaultValue={app.name} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="app-url">URL</Label>
                <Input id="app-url" placeholder="https://www.example.com" required name="url" maxLength={150} minLength={10} defaultValue={app.url} />
              ...
              {params?.message && <p className="mt-4 p-4 rounded-sm  bg-foreground/10 text-foreground text-center text-red-500">{params.message}</p>}
            </div>
          </CardContent>
          <CardFooter>
